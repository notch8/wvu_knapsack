...
RUN RAILS_ENV=production SECRET_KEY_BASE=$(bin/rake secret) DB_ADAPTER=nulldb DB_URL='postgresql://fake' \
    bundle install && RAILS_ENV=production bundle exec rake assets:precompile && yarn install
